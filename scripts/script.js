...
async function loadComments(slug) {
  try {
    const resp = await fetch(`${API_BASE}/comments?slug=${encodeURIComponent(slug)}`, { cache: "no-store" });
...
async function saveComment(slug, { name, text }) {
...
async function renderComments(slug) {
  ...
  if (!container) return;
  const list = await loadComments(slug);
...
function initCommentForm(slug) {
  ...
  form.onsubmit = async (e) => {
    ...
    await saveComment(slug, { name, text });
